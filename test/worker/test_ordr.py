...
def test_wait_and_set_video_url(
    enqueue, success, set_video, set_video_progress, get_render_id, logging
):
    score_id = 12345678
    ...
    enqueue.assert_called_with(
        ANY, score_id, render_id, comment, wait=timedelta(seconds=2)
    )
    reset_mock()
